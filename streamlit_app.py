...
def load_and_process_data():
    # ************************************************
    # ** 파일 목록 수정: 단순화된 파일명으로 변경 **
    # ************************************************
    # 데이터 연도와 일치하는 단순한 파일명(예: 2020.xlsx)으로 변경
    files = [
    ...
    for item in files:
        ...
        try:
            # 1. 헤더 처리 및 데이터 로드 (연도별 헤더 위치 다름)
            # 2023년(2022년 실적) 파일부터 헤더 위치가 달라짐
            if item['year'] >= 2023:
...
# -----------------------------------------------------------------------------
if df.empty:
    st.error("😭 데이터를 추출하지 못했습니다. **data** 폴더 안에 파일명이 `2020.xlsx`, `2021.xlsx`, `2022.xlsx`, `2023.xlsx`, `2024.xlsx`로 있는지 확인해 주세요.")
    st.stop() 
